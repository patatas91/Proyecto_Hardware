...
#define  DEBUGEND (DEBUGSTACK-DEBUGSIZE) // fin de la pila
#define DEBUGSTACK (HEAPEND) // inicio de la pila (final de la otra)
#define DEBUGSIZE 0x100 // tamaño de la pila 256
//#define DEBUGSIZE 0x12C // tamaño de la pila 300
unsigned int * pointer = (unsigned int *) DEBUGSTACK; // puntero al inicio de la pila
extern int timer2_Leer(); // lectura del timer2
...
//--------------------------------DEBUGSTACK---------------------------//
/*
 * Inicializamos los valores de la pila de depuracion a 0
 */
void init_debugstack(){
	while(pointer > (unsigned int *) DEBUGEND) {
		// mientras no lleguemos a la final vamos poniendo 0
		pointer = pointer - 3;
		(pointer[0]) = 0;
		(pointer[1]) = 0;
		(pointer[2]) = 0;
	}
	//ponemos el puntero al incio de la pila
	pointer = (unsigned int *) DEBUGSTACK;
}

void push_debug(int ID_evento, int auxData) {
	// comprobamos si hemos llegado a la cima
	pointer = pointer - 3;
	if(pointer < (unsigned int *) DEBUGEND) {
		// hemos llegado, volvemos al inicio
		pointer = (unsigned int *) DEBUGSTACK;
		pointer = pointer - 3;
	}
	// apilamos los datos
